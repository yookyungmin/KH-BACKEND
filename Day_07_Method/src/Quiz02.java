...
//	}
	...
	public static int countJava(String str) {
		int count =0; //카운트 변수
		String[] arr = str.split(":");   //split은 문자열을 배열로 짜름
		System.out.println(arr.length);
		
		for(int i=0; i<arr.length; i++) { // arr.length 배열의 칸수 // str.length 문자열의 길이
...
//	
//	public static int countJavaa(String s) {
//		int count = 0;
//		String[] arr = s.split(":");
//		System.out.println(arr.length);
//		
...
//	public static int countJava(String str) {
//
//        String find_str = "Java";
//
//        String[] arr_str = str.split(find_str);
//        int count = arr_str.length - 1;
//        return count;
//    }
...
//		String[] arr_str = str.split(find_str);
//		int count = arr_str.length-1;
//		if(str.endsWith(find_str)) {
//			count++;
//		}
//		return count;
//	}
	
	public static void main(String[] args) {  //main도 메서드 개발자가 콜하는게 아니라 사용자의해 콜되는 메서드 매개변수는 프로그램바깥에서 전달되어 온다
