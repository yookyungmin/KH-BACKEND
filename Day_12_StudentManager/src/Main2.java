...
public class Main2 {
	...
	public static void printStudents(ArrayList<Student> stds) { //출력 메서드
		System.out.println("학년\t학번\t이름\t국어\t영어\t수학\t합계\t평균");
		for(Student std : stds) {
			System.out.println(std.getGra()+"\t"+
								std.getId()+"\t"+
								std.getName()+"\t"+
								std.getKor()+"\t"+
								std.getEng()+"\t"+
		...
		while(true) {
			...
			if(menu==1) {
				...
				if(grade ==1) {
					...
					manager2.addMember(new Freshman(grade, id, name, math, kor, eng));
					continue;
				}catch (Exception e) {
					...
					try {
						...
						manager2.addMember(new Junior(grade, id, name, kor, eng, math));
						continue;
					}catch (Exception e) {
						System.out.println("제대로 입력하세요");
				...
				ArrayList<Student> stds = manager2.getMembers();
				...
				ArrayList<Student> mem = manager2.searchName(sname);
				
				if(mem.size()==0) {
					System.out.println(sname+"학생이 존재 하지 않습니다");
				}else {
					printStudents(mem);
				}
			...
			Student std = new Student(grade,id, null, kor, eng, math);
			manager2.updateById(std);
		}else if(menu==6){
